d3.csv("data/life_expectancy_by_country.csv")
  .then((data) => {
    const getDiff = (arr) => {
      const unique = [...new Set(arr.map((item) => item.iso_code))];
      const ret = [];

      for (i in unique) {
        const country = unique[i];
        const getit = arr.filter((el) => el.iso_code == country);
        const value_list = [...getit.map((item) => +item.total_cases)];
        const diff = Math.max(...value_list) - Math.min(...value_list);
        const map = {};
        map["iso_code"] = country;
        map["gap"] = diff;
        ret.push(map);
      }
      return ret;
    ...
    const condition = [...gapmap.slice(0, 5).map((item) => item.iso_code)];
    const processedData = data.filter((el) => condition.includes(el.iso_code));
